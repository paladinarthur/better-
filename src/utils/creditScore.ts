...
export function calculateCreditScore(userData: UserData) {
    // First parse all numeric values
    const annualIncome = Number(userData.annualIncome.replace(/,/g, ''));
    const avgCreditCardUsage = Number(userData.avgCreditCardUsage.replace(/,/g, ''));
    const age = Number(userData.age);

    let score = 300; // Base score (Minimum)

    // 1. Income & Loan Utilization (40%)
    const totalLoans = userData.previousLoans.reduce((sum, loan) => {
        const loanAmount = Number(loan.loanAmount.replace(/,/g, ''));
        return sum + loanAmount;
    }, 0);
    
    const utilization = ((totalLoans + avgCreditCardUsage * 12) / annualIncome) * 100;
...
export function getLoanEligibility(creditScore: number, loanType: string) {
    const baseRates = {
        home: {
            baseRate: 8.50,
            maxAmount: 10000000
